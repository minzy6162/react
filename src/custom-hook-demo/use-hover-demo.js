...
// 노드가 정상적으로 존재할 경우 mouseover, mouseout 이벤트 핸들러 설정
        ...
        // 호버 여부를 검사할 DOM 요소가 변경될 경우에만 다시 이벤트 설정하도록 의존 배열설정
            [ref.current]
    );
    return [ref, value];
}
...
// 필요한만큼 호출 가능
    ...
    return (
        <div>
            <div ref={faceHoverRef} style={{ fontSize: "80px"}} >{isFaceHovered
                ? "😁" : "☹"}</div>
            <div ref={colorHoverRef} style={{ width: "100px", height: "100px",
                background: `${isColorBoxHovered ? 'yellow' : 'lightgray'}` }}>
                {isColorBoxHovered ? <span>hovered</span> : <span>not
hovered</span>}
            </div>
        </div>
    )
